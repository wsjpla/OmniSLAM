...
启动docker：
```
$ sudo bash start_docker.bash 1
# 选参数1，把当前文件夹映射到docker容器中
```
...
定位：
```
$ roslaunch d2vins seeker.launch
```
定位相关参数在 `config/seeker_tmp/quadcam_single.yaml`


深度估计：
```
$ roslaunch quadcam_depth_est depth-node-seeker.launch
```
深度估计相关参数在 `config/seeker_tmp/quadcam_depth.yaml`
